...
* Fetch upstream changes
  ...
  ```sh
  git remote add upstream git@github.com:greenbone/autohooks-plugin-flake8.git
  git fetch upstream
...
* Create a source and wheel distribution:
  ...
  ```sh
  rm -rf dist build autohooks-plugin-flake8.egg-info pip-wheel-metadata
  poetry build
  ```
...
* Check if the package is available at <https://test.pypi.org/project/autohooks-plugin-flake8>

* Create a test directory

  ```sh
  mkdir autohooks-plugin-flake8-install-test
  cd autohooks-plugin-flake8-install-test
  git init
  python3 -m venv test-env
  source ./test-env/bin/activate
  pip install -U pip  # ensure the environment uses a recent version of pip
  pip install --pre -I --extra-index-url https://test.pypi.org/simple/ autohooks-plugin-flake8
  python -c "from autohooks-plugin-flake8.__version__ import __version__; print(__version__)"
  autohooks-plugin-flake8 check
  ```
...
* Remove test environment
  ...
  deactivate
  cd ..
  rm -rf autohooks-plugin-flake8-install-test
  ```
...
* Run pontos-release prepare
  ...
  ```sh
  poetry run pontos-release --release-version <version> --next-release-version <dev-version> --project autohooks-plugin-flake8 --space greenbone --git-signing-key <your-public-gpg-key> --git-remote-name upstream prepare
  ```
...
* Run pontos-release release
  ...
  ```sh
  poetry run pontos-release --release-version <version> --next-release-version <dev-version> --project autohooks-plugin-flake8 --space greenbone --git-signing-key <your-public-gpg-key> --git-remote-name upstream release
  ```
...
* Check if new version is available at <https://pypi.org/project/autohooks-plugin-flake8>

## Check the Release

* Check the Github release:

   See https://github.com/greenbone/autohooks-plugin-flake8/releases
